...
const AddressTxs = ({ shouldRender = true, isQueryEnabled = true, strictMode = false }: Props) => {
  ...
  const filter = strictMode ? undefined : (
    <AddressTxsFilter
  ...
  const csvExportLink = strictMode ? null : (
  ...
  return (
    <>
      ...
      <TxsWithAPISorting
        ...
        top={ strictMode ? 0 : ACTION_BAR_HEIGHT_DESKTOP }
        sorting={ sort }
        setSort={ setSort }
        showTableViewButton={ !strictMode }
      />
    </>
  );
};
