...
import StrictTransactionsPage from 'ui/sethStrict/StrictTransactionsPage';
...
const Transactions = () => {
  ...
  const txsPendingQuery = useQueryWithPages({
    ...
    options: {
      enabled: !isSethStrict && tab === 'pending',
  ...
  const txsWithBlobsQuery = useQueryWithPages({
    ...
    options: {
      enabled: !isSethStrict && config.features.dataAvailability.isEnabled && tab === 'blob_txs',
  ...
  const txsWatchlistQuery = useQueryWithPages({
    ...
    options: {
      enabled: !isSethStrict && tab === 'watchlist',
  ...
  const pageTitle = (() => {
    ...
    if (config.meta.seo.enhancedDataEnabled) {
      return `${ config.chain.name } transactions`;
    }
    return 'Transactions';
  })();
  ...
  return (
    <Box className="seth-page-shell">
      { !isSethStrict && <PageTitle title={ pageTitle }/> }
      { isSethStrict ? (
        <StrictTransactionsPage query={ txsValidatedQuery }/>
      ) : (
        <>
          <RoutedTabs
            tabs={ tabs }
            listProps={ listProps }
            rightSlot={ rightSlot }
            stickyEnabled={ !isSethCompact && !isMobile }
          />
          <Box className="seth-panel seth-panel-soft" p={{ base: 3, lg: 4 }} mt={ 4 }>
            <TxsStats mb={ 0 }/>
          </Box>
        </>
      ) }
