...
import StrictAddressPage from 'ui/sethStrict/StrictAddressPage';
...
const AddressPageContent = () => {
  ...
  const pageTitle = (() => {
    const subject = addressQuery.data?.is_contract && addressQuery.data?.proxy_type !== 'eip7702' ? 'Contract' : 'Address';
    return `${ subject } details`;
  })();

  if (isSethStrict) {
    return <StrictAddressPage hash={ hash } addressQuery={ addressQuery }/>;
  }
  ...
  return (
    <Box className="seth-page-shell">
      <PageTitle
        ...
        isLoading={ isLoading }
      />
      { !addressMetadataQuery.isPending && <AddressAlerts tags={ addressMetadataQuery.data?.addresses?.[hash.toLowerCase()]?.tags }/> }
      { config.features.metasuites.isEnabled && <Box display="none" id="meta-suites__address" data-ready={ !isLoading }/> }
      <RoutedTabs tabs={ tabs } isLoading={ isTabsLoading } listProps={ isMobile ? undefined : TAB_LIST_PROPS }/>
      <TextAd mt={ 6 }/>
    </Box>
  );
};
