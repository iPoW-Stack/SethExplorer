...
import StrictBlockDetailPage from 'ui/sethStrict/StrictBlockDetailPage';
...
function parsePoolIndex(value: string | Array<string> | undefined): number | undefined {
...
const BlockPageContent = () => {
  ...
  const poolIndex = parsePoolIndex(router.query.pool_index);
  const multichainContext = useMultichainContext();

  const blockQuery = useBlockQuery({ heightOrHash, poolIndex });
  const blockTxsQuery = useBlockTxsQuery({ heightOrHash, blockQuery, tab, poolIndex });
  const blockWithdrawalsQuery = useBlockWithdrawalsQuery({ heightOrHash, blockQuery, tab, poolIndex });
  const blockDepositsQuery = useBlockDepositsQuery({ heightOrHash, blockQuery, tab, poolIndex });
  const blockBlobTxsQuery = useBlockBlobTxsQuery({ heightOrHash, blockQuery, tab, poolIndex });
  const blockInternalTxsQuery = useBlockInternalTxsQuery({ heightOrHash, blockQuery, tab, poolIndex });
  ...
  const tabs: Array<TabItemRegular> = React.useMemo(() => ([
    {
      ...
      component: (
        <>
          ...
          <BlockDetails query={ blockQuery } poolIndex={ poolIndex }/>
        </>
      ),
    },
    {
  ...
  if (isSethStrict) {
    return <StrictBlockDetailPage blockQuery={ blockQuery } heightOrHash={ heightOrHash }/>;
  }
