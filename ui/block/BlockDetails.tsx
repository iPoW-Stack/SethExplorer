...
function blockPageQuery(heightOrHash: string, poolIndex?: number | null) {
  const query: { height_or_hash: string; pool_index?: string } = { height_or_hash: heightOrHash };
  if (poolIndex != null && !heightOrHash.startsWith('0x')) {
    query.pool_index = String(poolIndex);
  }
  return query;
}

const BlockDetails = ({ query, poolIndex }: Props) => {
  ...
  const handlePrevNextClick = React.useCallback((direction: 'prev' | 'next') => {
    ...
    const query = blockPageQuery(nextId, poolIndex);

    router.push(routeParams({ pathname: '/block/[height_or_hash]', query }, { chain: multichainContext?.chain }));
  }, [ data, multichainContext, poolIndex, router ]);
  ...
  const txsNum = (() => {
    const baseQuery = { ...blockPageQuery(heightOrHash, poolIndex), tab: 'txs' as const };
    const blockTxsNum = (
      <Link href={ route({ pathname: '/block/[height_or_hash]', query: baseQuery }, multichainContext) }>
    ...
    const blockBlobTxsNum = (config.features.dataAvailability.isEnabled && data.blob_transaction_count) ? (
      <>
        <span> including </span>
        <Link href={ route({ pathname: '/block/[height_or_hash]', query: { ...blockPageQuery(heightOrHash, poolIndex), tab: 'blob_txs' } }, multichainContext) }>
    ...
    <DetailedInfo.Container templateColumns={{ base: 'minmax(0, 1fr)', lg: 'minmax(min-content, 200px) minmax(0, 1fr)' }} >
      ...
      { config.features.beaconChain.isEnabled && Boolean(data.withdrawals_count) && (
        <>
          ...
          <DetailedInfo.ItemValue>
            <Skeleton loading={ isPlaceholderData }>
              <Link href={ route({ pathname: '/block/[height_or_hash]', query: { ...blockPageQuery(heightOrHash, poolIndex), tab: 'withdrawals' } }, multichainContext) }>
