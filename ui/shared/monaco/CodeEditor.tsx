...
const CodeEditor = ({ data, remappings, libraries, language, mainFile, contractName }: Props) => {
  ...
  React.useEffect(() => {
    instance?.editor.setTheme(colorMode === 'light' ? 'seth-light' : 'seth-dark');
  ...
  const handleEditorDidMount = React.useCallback((editor: monaco.editor.IStandaloneCodeEditor, monaco: Monaco) => {
    ...
    monaco.editor.defineTheme('seth-light', themes.light);
    monaco.editor.defineTheme('seth-dark', themes.dark);
    monaco.editor.setTheme(colorMode === 'light' ? 'seth-light' : 'seth-dark');
