...
const HashStringShorten = ({ hash, noTooltip, as = 'span', type, tooltipInteractive, maxSymbols }: Props) => {
  ...
  if (hash.length <= charNumber) {
    return <chakra.span as={ as } fontFamily="mono" data-entity-hash="true">{ hash }</chakra.span>;
  }

  const content = <chakra.span as={ as } fontFamily="mono" data-entity-hash="true">{ shortenString(hash, charNumber) }</chakra.span>;
