...
const BlocksContent = ({ type, query, enableSocket = true, top }: Props) => {
  ...
  const socketEnabled = enableSocket && !isSethCompact;
  ...
  const channel = useSocketChannel({
    ...
    isDisabled: query.isPlaceholderData || query.isError || query.pagination.page !== 1 || !socketEnabled,
  });
  useSocketMessage({
    channel,
    event: 'new_block',
  ...
  const content = query.data?.items ? (
    <>
      <Box hideFrom="lg">
        { query.pagination.page === 1 && socketEnabled && (
      ...
      <Box hideBelow="lg">
        <BlocksTable
          ...
          isLoading={ query.isPlaceholderData }
          showSocketInfo={ query.pagination.page === 1 && socketEnabled }
  ...
  const desktopSethHeader = !isMobile && isSethCompact ? (
    <Flex
      alignItems="center"
      ...
      bgColor="rgba(10, 16, 20, 0.8)"
    >
      <Text fontSize="sm" color="text.secondary">
        Total of{ ' ' }
        <Text as="span" fontWeight={ 600 } color="text.primary">
          { Number(statsQuery.data?.total_blocks || 0).toLocaleString() }
        </Text>
        { ' blocks' }
      </Text>
      { query.pagination.isVisible && <Pagination my={ 0 } { ...query.pagination }/> }
    </Flex>
  ) : null;
