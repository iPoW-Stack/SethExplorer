...
const BlocksTableItem = ({ data, isLoading, enableTimeIncrement, animation, chainData }: Props) => {
  ...
  return (
    <TableRow animation={ animation }>
        ...
        { data.transactions_count > 0 ? (
          <Skeleton loading={ isLoading } display="inline-block">
            <Link href={ route({
              ...
              query: {
                height_or_hash: data.hash || String(data.height),
                ...(data.pool_index != null && !data.hash ? { pool_index: String(data.pool_index) } : {}),
                tab: 'txs',
              },
            }) }>
      ...
      { !isSethCompact && !isRollup && !config.UI.views.block.hiddenFields?.burnt_fees && (
        <TableCell >
          <NativeCoinValue
      ...
      { !isSethCompact && !isRollup && !config.UI.views.block.hiddenFields?.base_fee && data.base_fee_per_gas && (
        <TableCell isNumeric>
          <NativeCoinValue
