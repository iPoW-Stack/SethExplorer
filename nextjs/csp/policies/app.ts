...
const DEV_HMR_SOURCES = config.app.isDev ? [
  `ws://${ config.app.host || 'localhost' }:${ config.app.port || '3000' }/_next/webpack-hmr`,
  // keep legacy default for environments that still run dev server on 3000
