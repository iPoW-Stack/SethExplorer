...
const handler = async(nextReq: NextApiRequest, nextRes: NextApiResponse) => {
  ...
  let baseUrl = nextReq.headers['x-endpoint']?.toString() || appConfig.apis.general?.endpoint;
  const forwardedProto = nextReq.headers['x-forwarded-proto']?.toString()?.toLowerCase();
  if (forwardedProto === 'http' && baseUrl?.startsWith('https://')) {
