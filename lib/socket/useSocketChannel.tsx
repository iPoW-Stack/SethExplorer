...
export default function useSocketChannel({ topic, params, isDisabled, onJoin, onSocketClose, onSocketError, socketName }: Params) {
  ...
  const socketDisabled = isDisabled || (isSethStrict() && getSethStrictDataSource() === 'stub');
  ...
  useEffect(() => {
    ...
    return cleanUpRefs;
  }, [ onSocketClose, onSocketError, socket, socketDisabled ]);

  useEffect(() => {
    if (socketDisabled && channel) {
      channel.leave();
      setChannel(undefined);
    }
  }, [ channel, socketDisabled ]);

  useEffect(() => {
    if (!socket || socketDisabled || !topic || !channelRegistry) {
      return;
    }
  ...
  }, [ socket, topic, params, socketDisabled, onSocketError, channelRegistry ]);
