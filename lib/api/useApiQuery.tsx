...
import { getSethStrictDataSource, isSethStrict } from 'lib/settings/useSethStrict';
...
export interface Params<R extends ResourceName, E = unknown, D = ResourcePayload<R>> {
  ...
  strictStubPolicy?: 'inherit' | 'force-disable';
}
...
export default function useApiQuery<R extends ResourceName, E = unknown, D = ResourcePayload<R>>(
  resource: R,
  { queryOptions, pathParams, queryParams, fetchParams, logError, chain: chainProp, strictStubPolicy = 'inherit' }: Params<R, E, D> = {},
) {
  const apiFetch = useApiFetch();
  const multichainContext = useMultichainContext();
  const chain = chainProp || multichainContext?.chain;
  const shouldForceDisableInStrictStub =
    strictStubPolicy === 'force-disable' &&
    isSethStrict() &&
    getSethStrictDataSource() === 'stub';

  const strictOverrides = shouldForceDisableInStrictStub ? {
    enabled: false,
