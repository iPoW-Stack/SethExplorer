...
export function getDefaultColorTheme(colorMode: ColorMode) {
  const configDefault = config.UI.colorTheme.default;
  if (configDefault && configDefault.colorMode === colorMode) {
    return configDefault.id;
  }
  const fallback = colorMode === 'dark' ? 'dark' : 'light';
  return fallback;
}
...
export const COLOR_THEMES: Array<ColorTheme> = [
  ...
  },
  {
    ...
    colorMode: 'dark',
    hex: '#040608',
    sampleBg: 'linear-gradient(154deg, #040608 50%, rgba(0, 255, 163, 0.06) 330.86%)',
  },
];
